...
int StageNode::ExportModels(SceneModels & models)
{
	...
	for (Stg::ModelPosition * positionmodel: models.positionmodels)
	{
		...
		if(robotmodels_.find(name) == robotmodels_.end())
		{
			...
			robot.reset(new StageRobot(name.c_str()));
		...
		robot->initROS(n_, exportMode != ExportSimple);

		if(exportMode == ExportSimple)
		{
			ROS_INFO("export=simple stops further enumeration of the robots");
			break;
		}
	}
	...
	if(map_publish_period > 0)
	{
		...
		{
			map_pub_ = n_.advertise<nav_msgs::OccupancyGrid>("map", 10);
			map_info_pub_ = n_.advertise<nav_msgs::MapMetaData>("map_info", 10);
			map_srv_ = n_.advertiseService("dynamic_map", &StageNode::cb_getmap_srv, this);
			map_publish_timer_ = n_.createTimer(ros::Duration(map_publish_period), &StageNode::onMapPublish, this);
		}
	}
...
void StageNode::WorldCallback()
{
    ...
    PublishContext context =
    	{(use_common_root?root_frame_id:""), tf, isDepthCanonical, sim_time};

    for (RobotContainer::value_type& it: robotmodels_)
    {
    	it.second->updateControl(sim_time);
    }
